...
$query_members = "SELECT * FROM products WHERE id = '$productId'";
$result_members = mysqli_query($conn, $query_members);
$data_members = mysqli_fetch_object($result_members);
$stock=$data_members->stock;
$quantityOut =  $stock - $qty;

$queryproduct = "update products SET  stock = '$quantityOut' where id = '$productId'";
...
$query_members = "SELECT * FROM cart WHERE cart_id = '$productId'";
$result_members = mysqli_query($conn, $query_members);
$data_members = mysqli_fetch_object($result_members);
...
// Fetch the cart item details before deleting
...
$updateStockQuery = "UPDATE products SET stock = '$newStock' WHERE id = '$productId'";
...
class="btn btn-primary prime-btn">Add To
Cart</button>
</div>
</div>
</div>
...
var stock = parseInt(document.getElementById('stockField').value) || 0;
// Check if quantity exceeds stock
if (quantity > stock) {
alert('Quantity exceeds available stock. Adjusting to maximum available.');
this.value = stock; // Set quantity to maximum stock value
quantity = stock;
}
